    ...
    var methods = {
        ...
        editBot: function(bot, unsafe){
            ...
            return methods.deleteBot(bot.secret)
                .then(function(){
                    //if unsafe, don't validate (validation strips featured for example).
                    return unsafe ? Q(bot) : addBotValidator(bot);
                })
                .then(addBotToRedis);
        },
        ...
        getBotFromId: function(botId) {
            var hgetall = Q.nbind(client.hgetall, client);
            return hgetall(BOT + botId);
        },
        getBotsFromIds: function(botIds) {
            return Q.all(botIds.map(function(botId){
                return methods.getBotFromId(botId);
            }));
        },
        ...
        installBots: function(clientKey, botIds){
            var sadd = Q.nbind(client.sadd, client);
            console.log('installing bots:', [clientKey + INSTALLED_BOTS].concat(botIds));
            return sadd([clientKey + INSTALLED_BOTS].concat(botIds));
        },
        uninstallBots: function(clientKey, botIds){
            var srem = Q.nbind(client.srem, client);
            console.log('installing bots:', [clientKey + INSTALLED_BOTS].concat(botIds));
            return srem([clientKey + INSTALLED_BOTS].concat(botIds));
        },
        getInstalledBots: function(clientKey){
            var smembers = Q.nbind(client.smembers, client);
            return smembers(clientKey + INSTALLED_BOTS);
        },
        installFeaturedBots: function(clientKey){
            return methods.getAllBots().then(function(allBots){
                return _.filter(allBots, function(bot){
                    return bot.featured === "true";
                }).map(function(featuredBot){
                    return featuredBot.id;
                });
            }).then(function(featuredBotIds){
                methods.installBots(clientKey, featuredBotIds);
            });
        },
