...
var helpers = {
	...
	statusCheck(response:{status:number}):boolean {
		return response && response.status !== 1;
	},
	getPlayerInfo(accountId:number):any {
	...
	extractItems(player:{[key:string]:number}):Array<string> {
		var playerItems = []
		var item;
		for (var i = 0; i <= 5; i++) {
			item = +player["item_" + i];
	...
	getNthMatch(accountId:number, n:number) {
		var deferred = q.defer();
		if (!accountId) {
