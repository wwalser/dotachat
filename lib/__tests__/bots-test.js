...
describe('bots', function(){
    ...
    pit('Edit bot validates the bot', function(){
        //ensure that editing a bot returns a bot with non-mutated fields unchanged.
        var bots = createBotsLib();
        //unchanged fields
        var keyword, email, homepage;
        ...
        return createRandomBot(bots, 1).spread(function(bot){
            ...
            //should never get here, validation fails.
            expect(false).toEqual(true);
        }).catch(function(error){
            expect(error).toEqual("A valid email address is required.");
        });
    });

    pit("Edit bot unsafe doesn't validate the bot", function(){
        //ensure that editing a bot returns a bot with non-mutated fields unchanged.
        var bots = createBotsLib();
        //unchanged fields
        var keyword, email, homepage;
        ...
        return createRandomBot(bots, 1).spread(function(bot){
            bot.name = customName;
            bot.featured = "true";
            return bots.editBot(bot, true);
        }).then(function(editedBot){
            expect(editedBot.featured).toEqual("true");
        });
    });
    ...
    pit('Install bots works', function(){
        ...
        return createRandomBot(bots, 3).spread(function(bot1, bot2){
            ...
            return bots.getInstalledBots(clientKey).then(bots.getBotsFromIds);
    ...
    pit('Install featured bots works', function(){
        var bots = createBotsLib();
        var featuredBotName;
        var clientKey = 4321;
        return createRandomBot(bots, 3).spread(function(bot1, bot2){
            featuredBotName = bot2.name;
            bot2.featured = "true";
            return bots.editBot(bot2, true);
        }).then(function(){
            return bots.installFeaturedBots(clientKey)
        }).then(function(){
            return bots.getInstalledBots(clientKey).then(bots.getBotsFromIds);
        }).spread(function(installedFeaturedBot){
            expect(installedFeaturedBot.name).toEqual(featuredBotName);
        });
    });
});